...
- `send_zero_empty`:
    - (5) `ast_ready_o` сигнал произвольно выставляется в `x` (`5125 ns: ast_ready_o is unknown` и так далее). так как лог ошибок и так становится слишком большим, я заккоментировал проверку сигналов на `x` со стороны [ast_dmx_monitor](./ast_dmx_monitor.sv#L124). при желании проверку можно вернуть. проблемы с `x` наблюдаются также и с другими output сигналами
